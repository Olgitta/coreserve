...
const {create, getAll, getById, like, remove, unlike, update} = require('../../src/apis/comments/controller');
...
describe('Comments API Routes', () => {
    ...
    it('GET /api/comments/:id', async () => {
        ...
        const res = await request(app).get(`/api/comments/100`);
        expect(res.statusCode).toBe(200);
    });
    ...
    it('POST /api/comments/like/:id', async () => {
        like.mockResolvedValue({statusCode: 200});
    ...
    it('POST /api/comments/unlike/:id', async () => {
        unlike.mockResolvedValue({statusCode: 200});
